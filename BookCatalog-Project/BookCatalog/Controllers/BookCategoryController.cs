...
namespace BookStore.Controllers
{
    public class BookCategoryController : Controller
    {
       ...
       /* [HttpGet]
        public IActionResult List(string Empsearch)
        {
            ViewData["GetEmployeedetails"] = Empsearch;

            var empquery = from z in _bookCategoryService.GetAll() select z;
            if (!string.IsNullOrEmpty(Empsearch))
            {
                empquery = empquery.Where(z => z.Name.Contains(Empsearch));
            }

            //var model = empquery.AsNoTracking().ToList();
